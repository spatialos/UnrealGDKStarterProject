...
markStartOfBlock "Build the StarterProject"
  ...
  Game/Scripts/Build.bat "StarterProjectServer" "Linux" "Development" "StarterProject.uproject" --skip-codegen
  if [[ ! -f "spatial/build/assembly/worker/UnrealWorker@Linux.zip" ]]; then
    echo "Linux Server was not properly built."
    exit 1
  fi

  Game/Scripts/Build.bat "StarterProject" "Win64" "Development" "StarterProject.uproject" --skip-codegen
